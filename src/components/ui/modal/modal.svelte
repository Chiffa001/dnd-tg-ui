<script lang="ts">
  import CrossIcon from '@/components/icons/cross-icon.svelte';

  export let isOpen: boolean;
  export let onClose: () => void;
  export let hasCloser: boolean = false;
</script>
...
{#if isOpen}
  <div class="absolute top-0 left-0 flex h-full w-full items-center justify-center">
    ...
    <div
      ...
      class="animate-modal-in absolute bottom-[34px] left-1/2 z-50 flex
             min-h-[285px] w-full -translate-x-1/2 scale-95 transform items-center justify-center rounded-3xl border-[0.5px]
             border-[#0D0D17] bg-[#232331] p-3 text-white opacity-0
